...
class Config:
    ...
    loss_name: str = "bce"  # bce or focal
    focal_gamma: float = 2.0
    focal_alpha: Optional[float] = None  # None recommended when using pos_weight
    use_amp: bool = True
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--loss", default=None, choices=["bce", "focal"])
...
def load_config(args: argparse.Namespace) -> Config:
    ...
    if args.loss is not None:
        d["loss_name"] = args.loss
    if args.focal_gamma is not None:
        d["focal_gamma"] = args.focal_gamma
    if args.focal_alpha is not None:
        d["focal_alpha"] = args.focal_alpha
...
def verify_dataframes(train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame,
    ...
    for name, df in [("train", train_df), ("val", val_df), ("test", test_df)]:
        ...
        for label in labels:
            ...
            invalid = set(df[label].dropna().unique()) - {0, 1, 0.0, 1.0}
            if invalid:
                raise ValueError(f"{name}:{label} invalid values: {invalid}")
...
class FocalLossWithLogits(nn.Module):
    """
    Multilabel focal loss for logits.

    This implementation starts from BCEWithLogitsLoss(reduction='none') and then
    applies the focal factor (1 - p_t)^gamma. It supports pos_weight, which is
    useful for imbalanced multilabel CheXpert labels.

    Recommendation for this project:
    - use pos_weight_mode: sqrt
    - use focal_alpha: null
    - start with focal_gamma: 2.0
    """
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        focal_factor = torch.pow((1.0 - p_t).clamp(min=1e-6), self.gamma)
...
def build_criterion(cfg: Config, pos_weight: Optional[torch.Tensor]) -> nn.Module:
    loss_name = str(cfg.loss_name).lower().strip()
    ...
    if loss_name == "focal":
        print(f"[LOSS] FocalLossWithLogits gamma={cfg.focal_gamma} alpha={cfg.focal_alpha}")
...
def main() -> None:
    ...
    print(f"[CONFIG] loss={cfg.loss_name} focal_gamma={cfg.focal_gamma} focal_alpha={cfg.focal_alpha}")
